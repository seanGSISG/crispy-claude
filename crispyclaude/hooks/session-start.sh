#!/usr/bin/env bash
# SessionStart hook for crispyclaude plugin
...
# Read using-crispyclaude content
using_crispyclaude_content=$(cat "${PLUGIN_ROOT}/skills/using-crispyclaude/SKILL.md" 2>&1 || echo "Error reading using-crispyclaude skill")

# Escape outputs for JSON
using_crispyclaude_escaped=$(echo "$using_crispyclaude_content" | sed 's/\\/\\\\/g' | sed 's/"/\\"/g' | awk '{printf "%s\\n", $0}')
